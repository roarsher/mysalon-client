...
//   getSalonBookings,
// } from '../../services/api';
// import { formatPrice, formatDuration, statusStyle, timeAgo } from '../../utils/helpers';
// import Spinner from '../../components/common/Spinner';

// const TABS = ['Live Queue', 'Today\'s Bookings'];
...
//   const [showCancelModal, setShowCancelModal] = useState(null); // bookingId
...
//       const res   = await getSalonBookings(salonId, { date: today, limit: 50 });
...
//     const interval = setInterval(() => { fetchQueue(); fetchBookings(); }, 15000);
//     return () => clearInterval(interval);
//   }, [fetchQueue, fetchBookings]);
...
//       await fetchBookings();
//     } catch (err) {
//       toast.error(err.message || 'Could not serve next customer.');
...
//     if (!queue) return;
//     if (!queue.isPaused && !pauseReason.trim()) {
//       setShowPauseBox(true); return;
//     }
//     setPausing(true);
//     try {
//       const res = await toggleQueuePause(salonId, {
...
//       toast.success(res.data.message);
//     } catch (err) {
//       toast.error(err.message || 'Could not toggle pause.');
//     } finally { setPausing(false); }
//   };

//   // ── Cancel a booking from queue ────────────────────────────────────────────
...
//     avgServiceTime = 20,
//     isPaused = false,
//     pauseReason: activePauseReason = '',
//     totalServedToday = 0,
//     activeBookings = [],
//   } = queue || {};

//   // Find if someone is currently in_progress
...
//         <h1 className="text-lg font-semibold text-gray-900">Queue Manager</h1>
//       </div>
...
//             <span className={`w-2.5 h-2.5 rounded-full ${isPaused ? 'bg-yellow-400' : 'bg-secondary animate-pulse'}`} />
//             <span className="text-sm font-medium text-gray-800">
//               {isPaused ? `Paused — ${activePauseReason}` : 'Queue is Active'}
//             </span>
//           </div>
//           <span className="text-xs text-gray-400">Avg. {avgServiceTime} min/customer</span>
//         </div>
...
//               <div>
//                 <p className="text-xs text-yellow-600 font-medium mb-0.5">✂️ Currently serving</p>
//                 <p className="font-semibold text-gray-900">{inProgressBooking.user?.name}</p>
...
//                 className="bg-green-500 text-white text-xs px-3 py-2 rounded-lg hover:bg-green-600 transition font-medium"
//               >
//                 Done ✓
//               </button>
//             </div>
...
//             className="flex-1 bg-primary text-white py-3 rounded-xl font-semibold text-sm hover:bg-primary-dark disabled:opacity-40 disabled:cursor-not-allowed transition active:scale-[0.98]"
//           >
//             {serving ? (
...
//                 Calling…
//               </span>
//             ) : waitingCount === 0 ? 'Queue Empty' : `Call Next (${waitingCount} waiting)`}
//           </button>

//           <button
//             onClick={() => isPaused ? handlePauseToggle() : setShowPauseBox(true)}
//             disabled={pausing}
...
//             <p className="text-xs text-yellow-700 font-medium mb-2">Reason for pausing</p>
//             <input
//               value={pauseReason}
//               onChange={(e) => setPauseReason(e.target.value)}
//               placeholder="e.g. Lunch break, Technical issue…"
//               className="w-full border border-yellow-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:border-yellow-400 mb-2"
//             />
//             <div className="flex gap-2">
...
//                 className="flex-1 border border-yellow-200 text-yellow-600 text-sm py-2 rounded-lg hover:bg-yellow-50 transition">
//                 Cancel
//               </button>
//             </div>
//           </div>
...
//             }`}>
//             {t}
//             {t === 'Live Queue' && waitingCount > 0 && (
//               <span className="ml-1.5 bg-primary text-white text-xs px-1.5 py-0.5 rounded-full">{waitingCount}</span>
//             )}
//           </button>
//         ))}
//       </div>
...
//                       </p>
//                       {idx === 0 && (
//                         <span className="badge bg-primary text-white text-xs">Next up</span>
//                       )}
//                     </div>
...
//                       Token #{b.tokenNumber} · {formatPrice(b.totalAmount)} · {timeAgo(b.createdAt)}
//                     </p>
//                   </div>
...
//                     className="text-xs text-red-400 border border-red-100 px-2 py-1.5 rounded-lg hover:bg-red-50 transition flex-shrink-0"
//                   >
//                     ✕
//                   </button>
//                 </div>
...
//             <div className="text-center py-12">
//               <div className="text-5xl mb-3">📋</div>
//               <p className="text-gray-500">No bookings today yet.</p>
//             </div>
//           ) : (
//             <div className="space-y-2">
//               {bookings.map((b) => (
...
//                       <span className={`badge text-xs ${statusStyle(b.status)}`}>
//                         {b.status.replace('_', ' ')}
//                       </span>
//                     </div>
//                     <p className="text-xs text-gray-400 truncate">
...
//                     </p>
//                     <p className="text-xs text-gray-400 mt-0.5">
//                       #{b.tokenNumber} · {formatPrice(b.totalAmount)} ·{' '}
//                       <span className={b.paymentStatus === 'paid' ? 'text-secondary-dark' : 'text-yellow-600'}>
...
//                     {b.user?.phone && (
//                       <a href={`tel:${b.user.phone}`} className="text-xs text-primary mt-1 block hover:underline">
//                         📞 Call
//                       </a>
//                     )}
//                   </div>
...
//             <p className="text-sm text-gray-500 mb-4">This will remove the customer from the queue.</p>
//             <div className="mb-4">
...
//               >
//                 {cancellingId === showCancelModal ? 'Cancelling…' : 'Cancel Booking'}
//               </button>
//             </div>
//           </div>
//         </div>
...
export default function QueueManagerPage() {
  ...
  const {
    waitingCount = 0, estimatedWait = 0, avgServiceTime = 20,
