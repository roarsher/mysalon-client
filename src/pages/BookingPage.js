...
import { useParams, useLocation, useNavigate } from 'react-router-dom';
import { toast }  from 'react-toastify';
import { getServices, createBooking, getPricingPreview, getSalonStylists } from '../services/api';
import { initiateRazorpayPayment } from '../services/payment';
import { formatPrice, formatDuration } from '../utils/helpers';
import Spinner from '../components/common/Spinner';
...
// ── Stylist picker card ───────────────────────────────────────────────────────
function StylistPicker({ stylists, selected, onSelect }) {
  if (!stylists.length) return null;

  return (
    <div className="card p-4 mb-4">
      <h2 className="text-sm font-semibold text-gray-800 mb-1">Choose Stylist</h2>
      <p className="text-xs text-gray-400 mb-3">Optional — leave on "Any" to auto-assign</p>
      <div className="flex gap-3 overflow-x-auto pb-1 hide-scrollbar">
        ...
        <button
          ...
          className={`flex-shrink-0 flex flex-col items-center gap-1.5 p-3 rounded-2xl border-2 transition w-20 ${
            !selected ? 'border-primary bg-primary-50' : 'border-gray-100 hover:border-gray-200'
          }`}
        >
          <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center text-xl">🎲</div>
          <p className="text-xs font-medium text-gray-600 text-center leading-tight">Any</p>
        </button>
        ...
        {stylists.map((sty) => (
          <button
            ...
            className={`flex-shrink-0 flex flex-col items-center gap-1.5 p-3 rounded-2xl border-2 transition w-20 ${
              selected?._id === sty._id ? 'border-primary bg-primary-50' : 'border-gray-100 hover:border-gray-200'
            }`}
          >
            {sty.photo?.url
              ? <img src={sty.photo.url} alt={sty.name} className="w-12 h-12 rounded-full object-cover" />
              : <div className="w-12 h-12 rounded-full bg-primary-50 flex items-center justify-center text-xl">💈</div>
            }
            <p className="text-xs font-medium text-gray-700 text-center leading-tight truncate w-full">{sty.name.split(' ')[0]}</p>
            {sty.experience > 0 && <p className="text-xs text-gray-400 leading-none">{sty.experience}yr</p>}
          </button>
        ))}
      </div>

      {selected && selected.speciality?.length > 0 && (
        <div className="mt-3 pt-3 border-t border-gray-100">
          <p className="text-xs text-gray-500 mb-1.5">
            <strong>{selected.name}</strong> specialises in:
          </p>
          <div className="flex flex-wrap gap-1">
            {selected.speciality.map((sp) => (
              <span key={sp} className="text-xs bg-primary-50 text-primary px-2 py-0.5 rounded-full">{sp}</span>
            ))}
          </div>
          {selected.bio && <p className="text-xs text-gray-400 mt-1.5 italic">"{selected.bio}"</p>}
        </div>
      )}
    </div>
  );
...
export default function BookingPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [svRes, priceRes, styRes] = await Promise.all([
          getServices(salonId),
          getPricingPreview(salonId, serviceIds.join(',')),
          getSalonStylists(salonId),
        ]);
        const all = svRes?.data?.data || [];
        setServices(all.filter((s) => serviceIds.includes(s._id)));
        const pd = priceRes?.data?.data;
        if (pd) { setPricing(pd.pricing); setIsFirstBooking(pd.isFirstBooking); }
        setStylists(styRes?.data?.data || []);
      } catch { toast.error('Could not load booking details.'); }
      finally   { setLoading(false); }
    };
    load();
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res     = await createBooking({
        ...
        notes:         notes.trim(),
        paymentMethod,
        stylistId:     selectedStylist?._id || null,
      ...
      initiateRazorpayPayment(
        ...
        (errorMsg) => {
          setStep('idle'); setConfirming(false);
          toast.error(errorMsg || 'Payment cancelled.');
          navigate(`/queue/${booking._id}`, { state: { salonName, booking, paymentFailed: true } });
        }
      );
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6 pb-36">
      <button onClick={() => navigate(-1)} className="flex items-center gap-1 text-primary text-sm mb-5 hover:underline">← Back</button>
      <h1 className="text-xl font-semibold text-gray-900 mb-0.5">Confirm Booking</h1>
      <p className="text-sm text-gray-400 mb-5">{salonName}</p>
      ...
      <div className="card p-4 mb-4">
        ...
        <textarea value={notes} onChange={(e) => setNotes(e.target.value)}
          rows={3} maxLength={300}
          placeholder="e.g. Preferred style, allergies to products…"
          className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm resize-none focus:outline-none focus:border-primary transition"
        />
        <p className="text-right text-xs text-gray-300 mt-1">{notes.length}/300</p>
      </div>
      ...
      <div className="card p-4 mb-4">
        ...
        <div className="space-y-2">
          {PAYMENT_METHODS.map((pm) => (
            <label key={pm.value}
              className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition ${
                paymentMethod === pm.value ? 'border-primary bg-primary-50' : 'border-gray-100 hover:border-gray-200 bg-white'
              }`}>
              <input type="radio" name="paymentMethod" value={pm.value} className="hidden" checked={paymentMethod === pm.value} onChange={() => setPaymentMethod(pm.value)} />
              <span className="text-2xl flex-shrink-0">{pm.icon}</span>
              <div className="flex-1"><p className="text-sm font-medium text-gray-800">{pm.label}</p><p className="text-xs text-gray-400">{pm.sub}</p></div>
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition ${paymentMethod === pm.value ? 'border-primary' : 'border-gray-300'}`}>
                {paymentMethod === pm.value && <div className="w-2.5 h-2.5 rounded-full bg-primary" />}
              </div>
            </label>
          ))}
        ...
        {paymentMethod === 'online' && (
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
            {['UPI','Card','Netbanking','Wallet'].map((m) => <span key={m} className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded">{m}</span>)}
            <span className="ml-auto text-xs text-gray-400">🔒 Razorpay</span>
          </div>
        )}
        {paymentMethod === 'cod' && (
        ...
        <div className="max-w-lg mx-auto">
          ...
          {selectedStylist && (
            <p className="text-xs text-center text-primary mb-2 font-medium">
              💈 {selectedStylist.name} will serve you
            </p>
          )}
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-xs text-gray-400">{services.length} service{services.length !== 1 ? 's' : ''}</p>
              <p className="text-xl font-bold text-gray-900">{formatPrice(pricing?.totalAmount || 0)}</p>
              {pricing?.discountAmount > 0 && <p className="text-xs text-green-600">You save {formatPrice(pricing.discountAmount)}</p>}
            </div>
            <button
              onClick={handleConfirm}
              disabled={confirming}
            ...
            >
              {confirming
                ...
                : paymentMethod === 'cod' ? 'Confirm Booking' : `Pay ${formatPrice(pricing?.totalAmount || 0)}`
              }
            </button>
          </div>
          <p className="text-center text-xs text-gray-400">
            {paymentMethod === 'online' ? '🔒 Secured by Razorpay · UPI · Cards · Netbanking' : '💵 Pay in cash at the salon counter'}
          </p>
        </div>
      </div>
    </div>
...
function PricingCard({ pricing }) {
  ...
  return (
    <div className="card p-4 mb-4">
      <button onClick={() => setOpen((v) => !v)} className="w-full flex items-center justify-between text-sm font-semibold text-gray-800">
        <span>Price Breakdown</span>
        <div className="flex items-center gap-2">
          <span className="text-primary font-bold">{formatPrice(pricing.totalAmount)}</span>
          <span className={`text-gray-300 text-xs transition-transform ${open ? 'rotate-180' : ''}`}>▼</span>
      ...
      {open && (
        <div className="mt-3 pt-3 border-t border-gray-100 space-y-2">
          {rows.map((row, i) => (
            <div key={i} className={`flex justify-between text-xs ${row.highlight ? 'text-green-600 font-semibold' : 'text-gray-500'}`}>
              <span>{row.label}</span>
              <span>{row.note ? <span className="text-gray-400">—</span> : row.sign === '-' ? `− ${formatPrice(row.amount)}` : formatPrice(row.amount)}</span>
            </div>
          ))}
          <div className="flex justify-between text-sm font-bold text-gray-900 pt-2 border-t border-gray-200 mt-2">
            <span>Total Payable</span>
          ...
          {pricing.discountAmount > 0 && <p className="text-xs text-green-600 text-center font-medium">🎉 You're saving {formatPrice(pricing.discountAmount)}!</p>}
        </div>
      )}
    </div>
  );
